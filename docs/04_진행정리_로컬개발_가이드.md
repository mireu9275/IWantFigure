...
| 앱(Flutter) | 촬영 → 얼굴 블러 → 분석 → 사진 오버레이/3D/설명 → 보정·관찰 → 저장, 18개 유형 가이드와 유형별 3D 모션 데모(unknown 제외 17종)까지 구현(코드 기준, 얼굴 블러의 실기 동작은 아래 미검증 참고). `flutter test` **217건 통과**, `flutter analyze` **이슈 0** (2026-09-25 로컬 Windows 실측) |
| 서버(.NET 10) | `POST /api/v1/analyze`, `GET /healthz`, Mock/Gemini/Claude 프로바이더. `dotnet test` **116건 통과**, Release 빌드 **경고 0** (2026-09-25 로컬 실측). mock 서버 스모크 테스트 200 OK (실측) |
| CI | GitHub Actions에서 모든 브랜치 push·PR마다 Flutter analyze+test, .NET build+test. push마다 실행되며 지금까지 모두 성공(문서 작성 시점 5회) |
| **미검증** | Android **에뮬레이터** debug 빌드·설치·실행은 2026-09-25 로컬에서 확인(실측). 같은 날 사용자가 에뮬레이터에서 사람 사진을 올렸을 때 얼굴 모자이크가 적용된 것을 확인(ML Kit 동작). 모의 모드라 결과는 예시였음 → 결과 화면에 모의 안내 띠 추가. Android **실기**, iOS 빌드, 실제 AI 분석은 아직 확인하지 못함 |
...
| 유형별 가이드 | 18개 유형 × ko/ja/en: 요약, 알아보기, 이렇게 노리기, 기법, 팁, 철수 조건, 참고 비용(★). 홈 카드·결과 화면 제목·설명 탭에서 진입. **움직임으로 보기**: 17개 유형의 3D 모션 데모(집게 이동·하강·닫힘·상승, 경품 기울기·이동·낙하)가 반복 재생되고, 재생 중인 단계가 강조되며 단계 번호·이전/다음으로 그 단계만 다시 볼 수 있음(docs/02 3.7). 2026-09-25에 4本橋渡し·4本 平行＋ハの字·紐吊り 3개 유형 추가(AI 분류 계약 포함, 명칭·공략은 확인 필요) |
...
| 테스트·CI·도구 | 앱 217건, 서버 116건(네트워크 불필요), CI(Flutter 3.47.5 + .NET 10), 스크린샷 도구(합성 사진 + 모의 분석으로 ko/ja/en 17장), 가이드 데모 프레임 시트 도구 |
...
| `app/lib/models/analysis.dart` | 스키마와 1:1인 `AnalysisResult`, enum(`LayoutType` 18종 등), `NBox`/`Pt` |
...
| `app/lib/scene3d/` | `projection.dart`(카메라), `scene_painter.dart`(렌더러), `scene_view.dart`(제스처·애니메이션), `timeline.dart`(키프레임 타임라인) |
| `app/lib/guide/` | 가이드 모션 데모: `guide_demo.dart`(공통 빌더), `guide_demos.dart`(등록부), `demos/`(계열별 데모) |
...
| `app/lib/l10n/strings.dart`, `guide_content.dart` | ko/ja/en 문자열, 18개 유형 가이드 본문 |
| `app/test/` | 테스트 217건 |
| `app/tool/screenshots_test.dart` | 스크린샷 생성 도구(일반 테스트와 분리) |
| `app/tool/guide_demo_frames_test.dart` | 가이드 데모 키프레임 시트 생성 도구(일반 테스트와 분리, `build/guide_demo_frames/`) |
...
| `server/IWantFigure.Server.Tests/` | xUnit 116건 |
...
| 6 | 모의 샘플이 `bridge_parallel` 하나 | 나머지 17개 유형은 모의 시연 불가 |
...
2. 이름을 **`NotoSansKR.ttf`**, **`NotoSansJP.ttf`**로 바꿔 **`app/tool/fonts/`**에 둡니다. 이 폴더는 `.gitignore`에 들어 있어 커밋되지 않습니다.
   - PC에 이미 설치된 Noto 폰트를 복사해도 됩니다(실측: `C:\Windows\Fonts\NotoSansKR-VF.ttf`, `%LOCALAPPDATA%\Microsoft\Windows\Fonts\NotoSansJP-Regular.ttf`). 이 경우 클라우드에서 만든 PNG와 글꼴 렌더링이 조금 달라 화면이 바뀌지 않은 PNG도 변경으로 잡히므로, 의도한 이미지만 남기고 나머지는 `git restore`로 되돌립니다.
...
- [x] 3-1 도구 설치, `flutter doctor -v` 통과(Android 항목 초록) — 2026-09-25 로컬 확인
- [x] 저장소 클론, `git config user.name/user.email` 설정, `develop` 생성·push(3-2)
- [x] Claude Code `attribution` 설정(4장)
- [x] 로컬에서 `flutter analyze`(이슈 0) / `flutter test`(217건), `dotnet test`(116건) 통과 — Windows에서만 실패하던 테스트 2건(줄바꿈 `\r\n`, 경로 구분자)을 고치고, PC 전역 NuGet 설정의 영향을 막으려 `server/nuget.config` 추가
...
- [ ] 가이드 모션 데모를 실제 기계 영상과 비교해 움직임 보정. 가이드 본문 横落とし 2단계("앞 모서리는 왼쪽 아암… 발톱이 박스 밖에서 안으로 닫히도록")는 2本爪가 좌우로 닫히는 구조와 맞지 않는다는 검토 의견이 있음(확인 필요)
