...
이 문서는 클라우드 세션에서 해 온 작업을 한곳에 정리하고, 사용자 PC(Windows 기준, macOS 차이는 따로 표시)에서 이어서 개발하는 방법을 설명합니다.
...
| 항목 | 상태 |
|---|---|
| 저장소 | https://github.com/mireu9275/IWantFigure — 원격 브랜치는 `claude/japan-figure-gacha-ai-app-78bxf4` 하나뿐(develop·main 없음), 최신 커밋 `acd37c6`, 총 16커밋, PR 없음 |
| 앱(Flutter) | 촬영 → 얼굴 블러 → 분석 → 사진 오버레이/3D/설명 → 보정·관찰 → 저장, 15개 유형 가이드까지 구현. `flutter test` **116건 통과**, `flutter analyze` **이슈 0** (실측) |
...
| CI | GitHub Actions에서 모든 브랜치 push·PR마다 Flutter analyze+test, .NET build+test. 4회 실행 모두 성공 |
...
모든 커밋은 2026-09-24(UTC)에 올라갔습니다. 오래된 순서입니다.
...
| 앱 화면 흐름 | 홈(촬영/갤러리, 촬영 가이드 카드, 가이드 진입, 히스토리) → 최초 실행 동의 → 분석 중(준비 → 얼굴 가리기 → 서버 → 조준 계산, 오류별 메시지, 재시도·모의 분석 대체) → 결과(사진/3D/설명 3탭, 보정 모드, 경품 시트, 단계 카드, 관찰 6종 + 실행 취소, 집게 회전 선택, 성공/실패 저장) → 설정(모의 모드, 서버 URL, 앱 키, 언어, 경품 프리셋, 얼굴 자동 가리기). 히스토리에서 연 세션은 읽기 전용 |
...
| 개인정보 | ML Kit 온디바이스 얼굴 검출 + 순수 Dart 모자이크(isolate). 업로드·표시·저장 모두 블러본 사용. 최초 실행 동의, `blur_faces` 기본 ON. 서버는 EXIF/GPS 제거, 디스크 미저장, 이미지·키 미로깅 |
...
flowchart TD
  subgraph APP["앱 (Flutter)"]
    A["촬영 / 갤러리<br/>image_picker 장변 1456px, JPEG 85"] --> B{"최초 실행 동의"}
    B --> C["SessionController.analyze()"]
...
테스트 수 추이: 앱 10 → 58 → 63 → 82 → 91 → 116, 서버 73 → 113.
...
- 스크린샷 도구: 새 기본 폰트 경로(`app/tool/fonts/`)로 실행 → PNG 17장이 커밋된 `docs/images`와 바이트 단위로 동일
...
| 9 | Android 출시 설정 미완 | `applicationId`가 템플릿 값(TODO), release가 debug 키로 서명(TODO). `gradle.properties`의 `-Xmx8G`는 메모리 적은 PC에서 조정 |
| 10 | 앱 키는 공유 비밀 | 사용자 인증 아님. SharedPreferences 평문 저장 |
| 11 | docs/03 체크리스트 대부분 미완료 | 정책 URL 게시, 문의 이메일, 스토어 개인정보 라벨, 상표 검토 |
| 12 | 사소한 정리거리 | `guide_content.dart`의 'Emoji shown next to…' 주석과 `_EmojiBadge` 클래스명 |
| 13 | 브랜치 정리 필요 | 원격에 main/develop와 PR이 없음. 이력에 wip 커밋 2개 |
...
| Android SDK Platform | API 36 | compileSdk/targetSdk 36 | SDK Manager |
...
| **CMake** | 3.22.1 이상(최소 3.10) | 위 `jni` 빌드 | SDK Manager |
...
그다음 `C:\dev\flutter\bin`을 사용자 PATH에 추가하고 새 터미널에서 `flutter --version`으로 3.47.5 / Dart 3.13.4를 확인합니다. (태그 이름 `3.47.5`는 확인 필요. 공식 사이트의 stable zip을 받아도 되지만, 버전이 3.47.5보다 낮으면 `pub get`이 실패하고 더 높으면 analyze 규칙이 달라질 수 있습니다.)
...
`git log` 첫 줄이 `acd37c6 feat(app): 배치 유형별 기초 가이드 화면 …`이면 최신입니다. 이어서 develop을 만듭니다.
...
> 기존 16개 커밋에 Claude 표기(Co-Authored-By 등)가 들어 있는지는 확인 필요입니다. 확인하려면 `git log -3 --format=full`로 본문 끝을 보면 됩니다. 이미 push된 이력은 되도록 재작성하지 말고, 앞으로의 커밋만 본인 명의로 남기면 됩니다(4장의 attribution 설정 참고).
...
2. **Settings → Languages & Frameworks → Android SDK**에서:
   ...
   - SDK Tools: **Show Package Details**를 켜고 **NDK (Side by side) 28.2.13676358**, **CMake 3.22.1 이상**, **Android SDK Build-Tools 36.x**, **Android SDK Command-line Tools (latest)**, **Android SDK Platform-Tools**, **Android Emulator**
3. 라이선스 수락과 진단:
...
6. Android Studio에서 프로젝트를 열 때는 **`app/` 폴더**를 엽니다. `app/android`만 먼저 열면 `local.properties`·`gradlew`가 아직 없어 Gradle sync가 실패합니다(이 파일들은 gitignore 대상이고 `flutter pub get`이나 첫 빌드가 만듭니다).
...
1. 홈 → 촬영 또는 갤러리 → 최초 실행 동의 다이얼로그(동의 / 모의 모드로만 사용)
...
기대 결과: `{"status":"ok","provider":"mock"}`.

**API 키는 `dotnet user-secrets`에 넣습니다**(저장소에 들어가지 않음, 서버 csproj에 `UserSecretsId=iwantfigure-server`가 이미 있음):

```powershell
cd C:\dev\IWantFigure\server
dotnet user-secrets set "Gemini:ApiKey" "<Gemini API 키>" --project IWantFigure.Server
dotnet user-secrets set "Analysis:Provider" "gemini" --project IWantFigure.Server
dotnet user-secrets list --project IWantFigure.Server
dotnet run --project IWantFigure.Server
```

Claude를 쓰려면 `Claude:ApiKey`와 `Analysis:Provider`=`claude`로 바꿉니다. 지우기: `dotnet user-secrets remove "Gemini:ApiKey" --project IWantFigure.Server`. Windows 저장 위치는 `%APPDATA%\Microsoft\UserSecrets\iwantfigure-server\secrets.json`(확인 필요, macOS/Linux는 `~/.microsoft/usersecrets/iwantfigure-server/secrets.json` 실측).
...
cmd(**`set`은 한 줄에 하나씩**. `set X=Y && ...`로 쓰면 값 끝에 공백이 붙어 `Server__AppKey`가 401을 냄):

```bat
cd C:\dev\IWantFigure\server
set Analysis__Provider=claude
set Claude__ApiKey=<Anthropic API 키>
...
macOS/Linux(실측, `jq` 필요. macOS의 `base64`는 `-w0` 대신 `base64 -i sample.jpg`):

```bash
IMG=$(base64 -w0 sample.jpg)
jq -n --arg img "$IMG" '{image_base64:$img, mime:"image/jpeg", locale:"ko", hints:{claw_count:2}}' > request.json
...
Docker(선택, 빌드 컨텍스트는 저장소 루트, 확인 필요):
...
앱 **설정 → 모의 분석 모드 끄기**(끄기 전에는 서버 URL 입력란이 회색) → 서버 URL 입력. 서버에 `Server:AppKey`를 설정했다면 앱 키에도 같은 값을 넣습니다.
...
| 항목 | 값 |
|---|---|
| 배포 타깃 | iOS **15.5**(Podfile `platform :ios, '15.5'`, pbxproj 2곳). ML Kit가 15.5 이상을 요구하므로 낮추지 말 것 |
...
- 폰트가 없으면 오류 없이 건너뛰고 한글·일본어가 네모(□)로 찍힙니다.
...
| Gradle sync: `flutter.sdk not set in local.properties`, `gradlew` 없음 | `app/android`를 먼저 열었음(생성 파일은 gitignore 대상) | `app`에서 `flutter pub get` 먼저 실행, Android Studio에서는 `app/` 폴더 열기 |
| `NDK not configured`, `CMake … was not found`, NDK 설치 단계 실패 | `jni` 플러그인이 NDK·CMake로 C 코드를 빌드 | SDK Manager에서 NDK 28.2.13676358과 CMake 3.22.1+ 설치, 라이선스 수락 |
...
| B. Teleport | 로컬 터미널에서 `claude --teleport <세션 ID>` 또는 세션 안에서 `/teleport` | 클라우드 대화 기록·브랜치를 로컬로 가져옴 | 조건이 있음: git 작업 트리가 깨끗할 것, 같은 저장소, 브랜치가 원격에 push돼 있을 것, 같은 Anthropic 계정. 가져온 뒤 `claude/...` 브랜치가 체크아웃되므로 develop으로 바꿔야 함 |
| C. Remote Control | 로컬 폴더에서 `claude remote-control`(또는 세션 안에서 `/remote-control`) | 실행은 내 PC에서 하면서 claude.ai/code나 Claude 모바일 앱에서 보고 지시할 수 있음 | 실행 위치는 여전히 내 PC(PC가 켜져 있어야 함) |
...
4. 휴대폰·웹에서도 이어 보고 싶으면 같은 폴더에서:
   ...
   그러면 내 PC에서 도는 세션이 claude.ai/code 또는 Claude 앱에 나타납니다.
...
- 개인 전용 규칙은 `CLAUDE.local.md`(커밋하지 않음)나 사용자 전역 `~/.claude/CLAUDE.md`에 둘 수 있습니다.
- 규칙이 바뀌면 `CLAUDE.md`를 고쳐 커밋하면 다음 세션부터 반영됩니다.

### 커밋에 Claude 표기 끄기

`CLAUDE.md`의 지시는 "부탁"에 가깝고, 실제로 끄는 것은 **설정 파일의 `attribution`**입니다. Claude Code 설정 문서 기준으로 `attribution.commit`은 커밋 트레일러, `attribution.pr`은 PR 설명의 표기이며, **빈 문자열 `""`이면 숨깁니다**. 예전 `includeCoAuthoredBy`는 deprecated입니다.
...
- `attribution.sessionUrl`은 클라우드·Remote Control 커밋에 붙는 claude.ai 세션 링크를 끄는 항목입니다. 값 형식은 확인 필요입니다.
- 커밋 작성자 자체는 `git config user.name` / `user.email`(3-2)로 정해집니다. 첫 커밋 뒤 `git log -1 --format=full`로 본인 명의이고 트레일러가 없는지 확인하세요.
...
3. 서버 연결:
   ...
   > server를 mock 프로바이더로 띄우고 healthz를 확인한 뒤, 에뮬레이터 앱에서 모의 모드를 끄고 http://10.0.2.2:8080 으로 실제 HTTP 경로가 동작하는지 같이 점검하자. 끝나면 docs/04 5장 체크리스트에서 완료된 항목을 정리해줘. 커밋은 내가 확인한 뒤에 내 명의로만 해.
...
- [ ] Android 에뮬레이터에서 `flutter run`(모의 모드): 홈 → 동의 → 촬영/갤러리 → 결과 3탭 → 보정 → 관찰 → 저장 → 히스토리 재열기
...
| user-secrets 위치 | `%APPDATA%\Microsoft\UserSecrets\iwantfigure-server\`(확인 필요) | `~/.microsoft/usersecrets/iwantfigure-server/` |
| 에뮬레이터 이미지 | x86_64 | Apple Silicon은 arm64-v8a |
| iOS 빌드 | 불가 | Xcode + CocoaPods로 가능 |
| Claude Code 설치 | `irm https://claude.ai/install.ps1 \| iex` | 공식 설치 스크립트(확인 필요) |
