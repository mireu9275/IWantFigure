...
| 앱(Flutter) | 촬영 → 얼굴 블러 → 분석 → 사진 오버레이/3D/설명 → 보정·관찰 → 저장, 15개 유형 가이드와 유형별 3D 모션 데모(unknown 제외 14종)까지 구현(코드 기준, 얼굴 블러의 실기 동작은 아래 미검증 참고). `flutter test` **194건 통과**, `flutter analyze` **이슈 0** (2026-09-25 로컬 Windows 실측) |
...
| **미검증** | Android **에뮬레이터** debug 빌드·설치·실행(최초 동의 창까지)은 2026-09-25 로컬에서 확인(실측). Android **실기**, iOS 빌드, 에뮬레이터에서의 전체 흐름(촬영 → 저장)은 아직 확인하지 못함 |
...
| 유형별 가이드 | 15개 유형 × ko/ja/en: 요약, 알아보기, 이렇게 노리기, 기법, 팁, 철수 조건, 참고 비용(★). 홈 카드·결과 화면 제목·설명 탭에서 진입. **움직임으로 보기**: 14개 유형의 3D 모션 데모(집게 이동·하강·닫힘·상승, 경품 기울기·이동·낙하)가 반복 재생되고, 재생 중인 단계가 강조되며 단계를 누르면 그 장면으로 이동(docs/02 3.7) |
...
| 테스트·CI·도구 | 앱 194건, 서버 113건(네트워크 불필요), CI(Flutter 3.47.5 + .NET 10), 스크린샷 도구(합성 사진 + 모의 분석으로 ko/ja/en 17장), 가이드 데모 프레임 시트 도구 |
...
- [x] 로컬에서 `flutter analyze`(이슈 0) / `flutter test`(194건), `dotnet test`(113건) 통과 — Windows에서만 실패하던 테스트 2건(줄바꿈 `\r\n`, 경로 구분자)을 고치고, PC 전역 NuGet 설정의 영향을 막으려 `server/nuget.config` 추가
