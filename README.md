...
5. 18가지 배치 유형별 **기초 가이드**(알아보기 → 이렇게 노리기 → 기법 → 팁 → 철수 조건)를 홈과 결과 화면에서 바로 열 수 있습니다. 판별 불가를 뺀 17개 유형은 **3D 모션 데모**로 집게와 경품이 어떻게 움직이는지 보여 주고, 이해가 안 되는 단계는 단계별로 골라 다시 볼 수 있습니다.
