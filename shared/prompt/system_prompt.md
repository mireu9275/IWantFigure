...
2. Locate the important objects with bounding boxes: every prize box/plush that is the target, the bars (each bar separately; pink/rubber tube bars as tube_bar), the claw unit, the drop hole/exit area, rings/tags, shelves.
...
- bridge_parallel (橋渡し・平行): one box resting across two parallel bars (front bar 手前バー and back bar 奥バー). Aim NOT at the centre: hook one claw tip just inside the box end (端ギリギリ) and alternate left/right so the box rotates and drops between the bars. Heavy box → tate_hame (stand it vertically), light box → yoko_hame (turn it flat).
